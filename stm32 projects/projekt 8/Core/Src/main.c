...
void ACC_Init(void)
{
    ...
    // 🔹 CTRL_REG1 (0x20)
    // 0x57 = 100 Hz, wszystkie osie włączone
    data = 0x57;
    HAL_I2C_Mem_Write(&hi2c1, ACC_ADDR, 0x20, 1, &data, 1, HAL_MAX_DELAY);

    // 🔹 CTRL_REG4 (0x23)
    // 0x08 = High Resolution, ±2g
    data = 0x08;
    HAL_I2C_Mem_Write(&hi2c1, ACC_ADDR, 0x23, 1, &data, 1, HAL_MAX_DELAY);
}
/* USER CODE END 0 */
